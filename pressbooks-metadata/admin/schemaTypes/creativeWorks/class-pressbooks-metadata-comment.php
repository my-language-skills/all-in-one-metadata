...
/**
 * The class for the comment type including operations and metaboxes
 *
...
class Pressbooks_Metadata_Comment extends Pressbooks_Metadata_Type{

	public function __construct($type_level_input) {
		parent::__construct($type_level_input);
		$this->class_name = __CLASS__ .'_'. $this->type_level;
		$this->type_settings =  array('comment_type' => array('Comment Type','http://schema.org/Comment'));
	...
	private function pmdt_add_metabox($meta_position){
		//----------- metabox ----------- //
		x_add_metadata_group( 	'comment-type', $meta_position, array(
			'label' 		=>	'Comment Type Properties',
			'priority' 		=>	'high',
		) );
		//----------- metafields ----------- //
		// Down Vote Count
		x_add_metadata_field( 	'pb_downvotecount_'.$meta_position, $meta_position, array(
			'group' 		=> 	'comment-type',
			'label' 		=> 	'Down Vote Count',
			'description'	=>	'The number of downvotes this question, answer or comment has received from the community.',
		) );
		// Parent Item
		x_add_metadata_field( 	'pb_parentitem_'.$meta_position, $meta_position, array(
			'group' 		=>	'comment-type',
			'label' 		=>	'Parent Item',
			'description'	=>	'The parent of a question, answer or item in general.',
		) );
		// Up Vote Count
		x_add_metadata_field( 	'pb_upvotecount_'.$meta_position, $meta_position, array(
			'group' 		=>	'comment-type',
			'label' 		=>	'Up Vote Count',
	...
	/**
	 * A function that creates the metadata for the comment type.
	 * @since 0.8.1
	 *
	 */
	public function pmdt_get_metatags() {
		...
		$comment_data = array(
			...
			'downvoteCount' => 'pb_downvotecount',
			'parentItem' => 'pb_parentitem',
			'upvoteCount' => 'pb_upvotecount'
		);

		$html = "<!-- Microtags --> \n";

		$html .= '<div itemscope itemtype="http://schema.org/Comment">';

		foreach ( $comment_data as $itemprop => $content ) {
