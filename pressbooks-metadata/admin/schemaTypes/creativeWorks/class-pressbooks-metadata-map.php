...
/**
 * The class for the map type including operations and metaboxes
 *
...
class Pressbooks_Metadata_Map extends Pressbooks_Metadata_Type {

	public function __construct($type_level_input) {
		parent::__construct($type_level_input);
		$this->class_name = __CLASS__ .'_'. $this->type_level;
		$this->type_settings = array('map_type' => array('Map Type','http://schema.org/Map'));
	...
	/**
	 * The function which produces the metaboxes for the map type
	...
	private function pmdt_add_metabox($meta_position){
		//----------- metabox ----------- //
		x_add_metadata_group( 	'map', $meta_position, array(
			'label' 		=>	'Map Type Properties',
			'priority' 		=>	'high',
		) );
		//----------- metafields ----------- //
		// Map Type
		x_add_metadata_field( 	'pb_map_type_'.$meta_position, $meta_position, array(
			'group' 		=>	'map',
			'label' 		=>	'Map Type',
	...
	/**
	 * A function that creates the metadata for the Map type.
	 * @since 0.8.1
	 *
	 */
	public function pmdt_get_metatags() {
		...
		$html .= '<div itemscope itemtype="http://schema.org/Map">';

		foreach ( $map_data as $itemprop => $content ) {
